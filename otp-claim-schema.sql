...
-- ---------- 1e. Real ownership linkage ----------
-- owner_name alone was just free text typed at claim time — anyone
-- scanning a claimed Genesis item saw the "you are the owner" experience,
-- regardless of who they actually were. owner_id links the claim to a
-- real Supabase Auth account, captured automatically at claim time
-- (claim.html now requires login before it will even look the item up),
-- so product.html can check "is the CURRENT viewer this exact account"
-- before showing owner-only content.
...
-- Customers can see their own claimed products (e.g. for a future "My
-- Collection" view). Redundant-but-harmless if anon/authenticated SELECT
-- on products is already unrestricted, which it is here.
...
-- ---------- 4. RPC: claim_product ----------
-- Step 2 of the customer claim flow. Requires the caller to be logged in
-- (claim.html now redirects to login.html?redirect=... before it will even
-- look the item up), an already-verified OTP for this qr_code, and a
-- matching serial number (serial_number if the admin generated one via the
-- "Assign Serial & Retailer" panel, otherwise falling back to item_number
-- for older products). Writes owner_name + owner_id + registered_at and
